"""上传图片到静态存储，获取公开访问 URL。

用法:
  python3 tools/image_upload.py --file /tmp/tyclaw_xxx/chart.png
"""
...
def upload(filepath: str) -> dict:
    ...
    upload_url = _PROXY_URL.replace("/api/dingtalk-proxy", "/api/upload-image")
    payload = json.dumps({
        "token": _PROXY_TOKEN,
...
def main():
    parser = argparse.ArgumentParser(description="上传图片获取公开 URL")
